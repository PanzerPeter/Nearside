...
> **The committed recipe no longer matches the table above.** Every result on this page comes from
> that 175k-batch run and stands as published. `config/training.yaml` now targets 600,000 batches at
> 28,000 frames over the paired 2× speed-perturbed corpus — 45,700 h, **23.6 passes over 1,933 h**,
> ~32 h wall — which is the direct attack on the gap this paragraph describes. The first attempt at
> that run was abandoned at step 419k and is being restarted from scratch (2026-08-05) after two
> spurious "gradient runaway" aborts and a weight-decay fix; no number here reflects it yet.
...
for the four dev/test splits — **157 GiB total**. Check `df -h` before starting.
