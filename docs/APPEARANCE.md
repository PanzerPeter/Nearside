...
Every theme lives in two places and the two must agree: an
`@plugin "daisyui/theme"` block in `src/index.css`, and an entry in `PACKS` or
`FREE_THEMES` in `src/lib/purchases.ts`. A pack's `id` is simultaneously its
